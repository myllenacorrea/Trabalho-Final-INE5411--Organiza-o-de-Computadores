...
# Cupom fiscal (Strings corrigidas para formata��o precisa)
...
main:
    ...
    sw $t5, 0($t1)                 # Preços dos combustiveis para o padrao
    sw $t5, 0($t2)
    sw $t5, 0($t3)
...
menu_principal:
    ...
    jal ler_menu_labsim
    move $t0, $v0                  # Procedimento para entrada pelo dls - escolha em t0
...
menu_cliente:
	...
	jal ler_menu_labsim
	move $t0, $v0               # Procedimento para entrada pelo dls - escolha em t0
	beq $t0, 0, menu_principal  # Pular p/ mostrar preços e metodo ou voltar p/ menu principa
...
escolher_modo:
	...
	la $a0, msg_metodo         # Carrega o endere�o da mensagem de m�todo (valor ou litro)
	syscall                    # Imprime a mensagem
	jal ler_menu_labsim        # L� a op��o do usu�rio
	move $t6, $v0              # Move a op��o para $t6
	beq $t6, 1, por_valor      # Se 1, vai para a fun��o de abastecer por valor
	beq $t6, 2, por_litros     # Se 2, vai para a fun��o de abastecer por litros
	j invalido                 # Se for outra op��o, trata como inv�lida

# --- Fun��es de Leitura do Teclado e Controle de LED (LabSim) ---
ler_menu_labsim:
	# Carrega os endere�os de hardware do LabSim em registradores
	li $t9, 0xFFFF0010         # Carrega o endere�o do display de 7 segmentos
	li $s6, 0xFFFF0012         # Carrega o endere�o para selecionar a linha do teclado
	li $s7, 0xFFFF0014         # Carrega o endere�o para ler a coluna do teclado

esperar_tecla:
	# Varre a linha 1 do teclado
	li $t1, 1                  # Carrega 1 para ativar a linha 1
	sb $t1, 0($s6)             # Escreve 1 no hardware, ativando a linha 1
	lbu $t0, 0($s7)            # L� o estado das colunas
...
# Fun��es que acendem o LED correspondente e retornam o n�mero da tecla
tecla_0:
	addi $sp, $sp, -4          # Abre espa�o na pilha para salvar o endere�o de retorno
	sw   $ra, 0($sp)           # Salva o registrador $ra na pilha
	lb   $t2, led_0            # Carrega o padr�o de bits do LED '0' para $t2
	sb   $t2, 0($t9)           # Envia o padr�o para o hardware, acendendo o LED '0'
	li   $v0, 0                # Define o valor de retorno da fun��o como 0
	jal  aguardar_soltar       # Chama a fun��o para esperar a tecla ser solta
	lw   $ra, 0($sp)           # Restaura o endere�o de retorno da pilha
...
tecla_1:
	...
	sw   $ra, 0($sp)           # Salva $ra
	lb   $t2, led_1            # Carrega o padr�o do LED '1'
...
tecla_2:
	addi $sp, $sp, -4          # Abre espa�o na pilha
	sw   $ra, 0($sp)           # Salva $ra
	lb   $t2, led_2            # Carrega o padr�o do LED '2'
...
tecla_3:
	addi $sp, $sp, -4          # Abre espa�o na pilha
	sw   $ra, 0($sp)           # Salva $ra
	lb   $t2, led_3            # Carrega o padr�o do LED '3'
...
# Fun��o que espera o usu�rio soltar a tecla para evitar leituras m�ltiplas (debouncing)
aguardar_soltar:
	li $t1, 1                  # Ativa a linha 1
	sb $t1, 0($s6)             # 
	lbu $t0, 0($s7)            # L� as colunas
	andi $t3, $t0, 0xF0        # Verifica se alguma coluna est� ativa (tecla pressionada)
	bnez $t3, aguardar_soltar  # Se estiver, a tecla ainda est� pressionada, ent�o espera
...
alterar_comum:
    ...
    li $v0, 1
    syscall
    li $v0, 4                     # Imprimir prompt alteração
    la $a0, prompt_alteracao
    syscall
    li $v0, 5                     # Ler inteiro 
    syscall
    sw $v0, preco_comum           # Guarda movo preco gasolina comum
    j menu_principal              # Redirecionamento p/ menu princpal
...
alterar_aditivada:
    ...
    li $v0, 5                     # Ler inteiro 
    syscall
    sw $v0, preco_aditivada       # Guarda movo preco gasolina aditivada
    j menu_principal              # Redirecionamento p/ menu princpa
...
alterar_alcool:
    ...
    li $v0, 5                     # Ler inteiro 
    syscall
    sw $v0, preco_alcool          # Guarda movo preco gasolina aditivada
    j menu_principal              # Redirecionamento p/ menu princpa

# Fun��es que preparam a simula��o com base na escolha do cliente
cliente_comum:
	lw $s0, preco_comum        # Carrega o pre�o da gasolina comum no registrador salvo $s0
	li $s3, 1                  # Define o tipo de combust�vel como 1 em $s3
	j escolher_modo            # Pula para a fun��o de escolher o m�todo de abastecimento

cliente_aditivada:
	lw $s0, preco_aditivada    # Carrega o pre�o da aditivada em $s0
	li $s3, 2                  # Define o tipo como 2 em $s3
	j escolher_modo

cliente_alcool:
	lw $s0, preco_alcool       # Carrega o pre�o do �lcool em $s0
	li $s3, 3                  # Define o tipo como 3 em $s3
	j escolher_modo

# L�gica para abastecer por valor em Reais
por_valor:
	...
	div $s1, $s2, $s0          # Divide o valor total ($s2) pelo pre�o ($s0) e guarda os litros em $s1
	j simular                  # Pula para a fun��o de simula��o

# L�gica para abastecer por quantidade de Litros
por_litros:
	li $v0, 4                  # Syscall para imprimir string
	...
	mul $s2, $s1, $s0          # Multiplica os litros ($s1) pelo pre�o ($s0) e guarda o valor total em $s2
	j simular                  # Pula para a fun��o de simula��o

# --- Simula��o e Gera��o de Cupom ---
simular:
	# Mostra na tela o resumo do abastecimento que vai come�ar
	li $v0, 4
	...
	syscall
	li $v0, 1
	move $a0, $s1              # Move os litros ($s1) para o argumento de impress�o
	...
	syscall
	li $v0, 1
	move $a0, $s2              # Move o valor total ($s2) para o argumento de impress�o
	syscall                    # Imprime o valor total
	li $v0, 4
	la $a0, str_linha          # Imprime a linha separadora
	syscall
	li $t7, 0                  # Zera o registrador $t7, que ser� o contador de litros abastecidos
...
loop_abastecimento:
	...
	li $v0, 32                 # Syscall para "dormir" (pausar a execu��o)
	...
	syscall
	li $v0, 1
	addi $a0, $t7, 1           # Adiciona 1 ao contador para mostrar o n�mero do litro atual (1, 2, 3...)
	syscall                    # Imprime o n�mero do litro
	li $v0, 4
	la $a0, str_L_final        # Imprime " L..."
	syscall
	addi $t7, $t7, 1           # Incrementa o contador de litros
	j loop_abastecimento       # Volta para o in�cio do loop

fim_simulacao:
	jal gerar_cupom            # Ao final, chama a fun��o para gerar o cupom fiscal
...
# ====================================================================
# gerar_cupom: Cria um cupom fiscal com as informa��es do abastecimento.
...
gerar_cupom:
    ...
    li $a2, 0                 # Sem permiss�es espec�ficas
    syscall
    move $s7, $v0             # Armazena descritor do arquivo em $s7

    # Escreve cabe�alho do cupom
    li $v0, 15
    move $a0, $s7
    ...
    # Escreve r�tulo do tipo de combust�vel
    li $v0, 15
    move $a0, $s7
    ...
    # Decide qual string de combust�vel escrever
...
# ================================================================
# escrever_alcool: Escreve "�lcool" no cupom
# ================================================================
escrever_alcool:
    li $v0, 15
    move $a0, $s7
...
continuar_cupom:
    ...
    # Restaura registrador de retorno e retorna da fun��o
    lw $ra, 0($sp)
    addi $sp, $sp, 4
    jr $ra

# ====================================================================
# int_to_string: Converte um n�mero inteiro positivo em string decimal.
# Entrada: $a0 = n�mero, $a1 = ponteiro para buffer
# Sa�da: buffer preenchido, $v1 = tamanho da string gerada
# ====================================================================
int_to_string:
    move $t8, $a0              # N�mero a converter
    move $t9, $a1              # Ponteiro para buffer
    li $t0, 10                 # Base 10
    li $t1, 0                  # Contador de d�gitos

    bnez $t8, itos_loop        # Se n�mero ? 0, vai para o loop
...
itos_loop:
    beqz $t8, itos_reverse     # Se n�mero for 0, termina loop
    divu $t8, $t0              # Divide por 10
    mflo $t8                   # Novo valor de t8 (quociente)
    mfhi $t2                   # Resto = d�gito atual
    addi $t2, $t2, '0'         # Converte para caractere ASCII
    sb $t2, 0($t9)             # Armazena no buffer
    addi $t9, $t9, 1           # Avan�a ponteiro do buffer
    addi $t1, $t1, 1           # Incrementa contador
    j itos_loop

itos_reverse:
    move $v1, $t1              # Salva o tamanho da string
    sub $t9, $t9, 1            # Corrige posi��o final do ponteiro
    move $t3, $a1              # In�cio do buffer
...
reverse_loop:
    ...
    sb $t4, 0($t9)
    addi $t3, $t3, 1           # Avan�a para o meio (esquerda)
    subi $t9, $t9, 1           # Avan�a para o meio (direita)
    j reverse_loop
...
# ====================================================================
# invalido: Exibe mensagem de erro para entradas inv�lidas e volta ao menu.
# ====================================================================
invalido:
    li $v0, 4
    la $a0, num_invalido
...
# ====================================================================
fim:
    li $v0, 10                 # Syscall 10: encerra execu��o
    syscall
